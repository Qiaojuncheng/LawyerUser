...
@property (nonatomic, strong) NSMutableArray *localAreaData;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    TLCityGroup *group = [self.data objectAtIndex:section - 4];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < 4) {
        ...
        cell.separatorInset = UIEdgeInsetsMake(0,cell.bounds.size.width *2 , 0, -cell.bounds.size.width);
         if(indexPath.section == 0){
            [cell setTitle:@"当前：无锡市崇安区"];
    ...
    TLCityGroup *group = [self.data objectAtIndex:indexPath.section - 4];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        return [TLCityGroupCell getCellHeightOfCityArray:self.localAreaData];
    } else if (indexPath.section == 1) {
...
//        if ([VC isKindOfClass:[QJMapSearchViewController class]]) {
//            [self.searchController.navigationController  popToViewController:VC animated:NO];
//         }
//    }
...
-(NSMutableArray *)localAreaData{


    _localAreaData =[[NSMutableArray alloc]initWithObjects:@"区域1",@"区域2",@"区域3",@"区域4",@"区域5", nil];
