...
-(void)shareWitType:(UMSocialPlatformType *)platformType{
    ...
    //创建网页内容对象
    NSString* thumbURL =[NSString stringWithFormat:@"%@%@",ImageUrl,self.model.avatar];
    NSData * imagedata  =[NSData dataWithContentsOfURL:[NSURL URLWithString:thumbURL]];
    UMShareWebpageObject *shareObject = [UMShareWebpageObject shareObjectWithTitle:self.model.name  descr:self.model.company thumImage:imagedata];
    //设置网页地址
    shareObject.webpageUrl = [NSString stringWithFormat:@"http://www.huirongfa.com/Wap/Lawyer/lawyer_xq/id/%@.html",self.model.id];
    //分享消息对象设置分享内容对象
